...
class OnlineTrainer():
    ...
    def calc_ELBO(self, sum_future_KL, sum_future_rewards, past_obs, past_action):     
        # *** Not numerically stable ***
        lik = (self.agent.Q_var(past_obs, past_action) - self.agent.Q_ref(past_obs, past_action)) **2
        if self.lik:    
            m_lik = np.mean((self.agent.Q_var_tab - self.agent.Q_ref_tab) **2)
            lik_m = lik - m_lik
        else:
            lik_m = 0
        if np.random.rand() < 0.001:
            print(lik, lik_m)
        return  - 0.5 * self.agent.PREC * self.agent.BETA * lik_m - int(self.explo_drive) / self.agent.BETA * sum_future_KL
    
    def online_ELBO(self, past_obs, past_action, obs, reward, done=False):      
        sum_future_KL = self.agent.Q_KL(past_obs, past_action)  
        sum_future_rewards = self.calc_sum_future_rewards(reward, obs, done)    
        return self.calc_ELBO(sum_future_KL, sum_future_rewards, past_obs, past_action)    
    ...
    def online_update(self, past_obs, past_action, obs, reward, done):
        ...
        for a in range(self.agent.N_act):
            self.agent.Q_var_tab[past_obs, a] -= self.agent.ALPHA * self.agent.Q_VAR_MULT * pi[a] * ELBO   
    ...
    def monte_carlo_update(self, done):
        ...
        for time in range(final_time):                
            ...
            if self.nb_trials > 10:
                ...
                pi = self.agent.softmax(past_obs)
                for a in range(self.agent.N_act):
                    self.agent.Q_var_tab[past_obs, a] -= self.agent.ALPHA *  pi[a] * ELBO   
