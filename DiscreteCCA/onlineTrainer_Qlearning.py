...
class OnlineTrainer():
    ...
    def calc_sum_future_rewards(self, reward, obs, done, actions_set=None):
        ...
        if not done:
            next_Q = self.agent.set_Q_obs(obs, Q=self.agent.Q_var)
            next_sum = np.max(next_Q)
            sum_future_rewards += self.agent.GAMMA * next_sum 
        if self.rtg_centering:
            sum_future_rewards -= np.mean(self.agent.Q_var_tab)
        return sum_future_rewards
    
    def calc_TD_err_ref(self, sum_future_rewards, past_obs, past_action):
        return self.agent.Q_var(past_obs, past_action) - sum_future_rewards
    ...
    def online_update(self, past_obs, past_action, obs, reward, done):
        ...
        if self.agent.do_reward:
            self.agent.Q_var_tab[past_obs, past_action] -= self.agent.ALPHA * self.online_TD_err_ref(past_obs,
                                                                                              past_action,
                                                                                              obs,
                                                                                              reward,
                                                                                              done=done)
    ...
    def monte_carlo_update(self, done):
        ...
        for time in range(final_time):                
            ...
            if self.nb_trials > 10:
                ...
                self.agent.Q_var_tab[past_obs, past_action] -= self.agent.ALPHA * TD_err_ref
